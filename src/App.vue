...
export default {
  ...
  methods: {
    checkData() {
      ...
      let yesterday = this.$moment().startOf('day').subtract(1, 'hour').format(format);
      // 今天的晚上九點，例：2020-03-23 09:00:00
      let today9PM = this.$moment().startOf('day').add(9, 'hour').format(format);
      
      console.log(now.isBetween(yesterday, today9PM));
